...
이 SDK 의 XCFramework 는 **Xcode 26.6 (Swift 6.3.3)** 으로 빌드했고 같은 버전에서 해석·빌드를 확인했습니다.

`.swiftinterface` 는 같거나 더 새로운 컴파일러가 읽는 것을 전제로 합니다. 따라서 **더 낮은 Xcode 에서는 모듈 임포트가 실패할 수 있습니다** — `failed to build module 'SPCPointHome'` 형태로 나타납니다. 하위 버전은 실측하지 않았습니다.

사용하시는 Xcode 버전을 알려주시면 확인하겠습니다. 필요하면 해당 버전으로 다시 빌드해 배포할 수 있습니다.

> 패키지 매니페스트 자체는 `swift-tools-version: 5.10` 이라 SwiftPM 요구사항은 낮습니다. 위 제약은 매니페스트가 아니라 **바이너리를 만든 컴파일러** 때문입니다.
